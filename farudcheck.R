data <- read.csv(file.choose())
View(data)
risky_good=ifelse(data$Taxable.Income<=30000,"risky","good")
fc=data.frame(data,risky_good)
View(fc)
library(caret)
library(C50)
# Data partion for model building and testing

inTraininglocal <- createDataPartition(fc$risky_good,p=.75,list=F)
training <- fc[inTraininglocal,]
View(training)
testing <- fc[-inTraininglocal,]
#model building
model <- C5.0(training$risky_good~.,data = training,trails = 40)
# Generating the model summary
summary(model)
pred <- predict.C5.0(model,testing[,-7])
a <- table(testing$risky_good,pred)
a
sum(diag(a)/sum(a))
plot(model)
###Bagging####
acc<-c()

###Bagging####
acc<-c()
for(i in 1:100)
{
  print(i)
  inTraininglocal<-createDataPartition(fc$risky_good,p=.85,list=F)
  training1<-fc[inTraininglocal,]
  testing<-fc[-inTraininglocal,]
  
  fittree<-C5.0(training1$risky_good~.,data=training1)
  pred<-predict.C5.0(fittree,testing[,-7])
  a<-table(testing$risky_good,pred)
  
  acc<-c(acc,sum(diag(a))/sum(a))
  
}
summary(acc)
acc

summary(acc)
acc

